...
sub viewServiceList
{
	...
	print Tr( th( {class => 'title', colspan => '7'}, "Running services on node $catchall_data->{name}" ) );
	...
	if ($processinventory)
	{
		...
		$processlist = $res->{data} if ($res->{success} && $res->{data});
	}

	if (ref($processlist) eq "HASH" && keys %$processlist)
	{
		print Tr(
