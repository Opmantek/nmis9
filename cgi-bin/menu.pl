...
sub menu_bar_site {
	...
	sub menu_site {
		...
		if ($AU->CheckAccess("table_config_view","check"))
		{
			@setupitems = (qq|<a id='cfg_setup' href="setup.pl?conf=$Q->{conf}&amp;act=setup_menu">Basic Setup</a>|,
											qq|------|,
											qq|<a id='cfg_groups' href="config.pl?conf=$Q->{conf}&amp;act=config_nmis_edit&amp;section=system&amp;item=group_list">Add/Edit Groups</a>|);
		}
		...
		push @setupitems, qq|<a id='cfg_nmis' href="config.pl?conf=$Q->{conf}&amp;act=config_nmis_menu&amp;section=system">System Configuration</a>|
				if ($AU->CheckAccess("table_config_view","check"));
		...
		push @setupitems, qq|<a id="cfg_models" href="models.pl?conf=$Q->{conf}&amp;act=config_model_menu&amp;model=Default&amp;section=threshold">Thresholding Alerts</a>| 
				if ($AU->CheckAccess("table_models_view","check"));				
		...
		push @menu_site, "Setup", \@setupitems if (@setupitems);
