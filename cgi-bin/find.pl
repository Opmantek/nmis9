	...
	foreach my $intHash (sortall2($II,'node','ifDescr','fwd')) {
		...
		if ( 	$II->{$intHash}{node} =~ /$qrfind/ or
			...
			if ($AU->InGroup($NT->{$II->{$intHash}{node}}{group})) {
				...
				push @out,Tr(
					td({class=>'info',nowrap=>undef},a({href=>"network.pl?conf=$Q->{conf}&act=network_node_view&node=$II->{$intHash}{node}&widget=$widgetstate"},$II->{$intHash}{node})),
					eval {
						if ($II->{$intHash}{collect} eq 'true') {
							return td({class=>'info'},a({href=>"network.pl?conf=$Q->{conf}&act=network_interface_view&node=$II->{$intHash}{node}&intf=$II->{$intHash}{ifIndex}&widget=$widgetstate"},$II->{$intHash}{ifDescr}));
						} else {
				...
				#	td({class=>'info'},$II->{$intHash}{ipAdEntNetMask}),
					td({class=>'info'},a({href=>url(-absolute=>1)."?act=find_interface_view&find=$II->{$intHash}{ipSubnet}&widget=$widgetstate"},$II->{$intHash}{ipSubnet})),
					td({class=>'info'},a({href=>url(-absolute=>1)."?act=find_interface_view&find=$II->{$intHash}{Description}&widget=$widgetstate"},$II->{$intHash}{Description})),
