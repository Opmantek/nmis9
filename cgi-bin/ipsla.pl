...
sub displayIPSLAmenu {
	...
	# get node info from /var, this file is produced by collect and update cycles
	my %RTTInfo = readVartoHash("nmis-nodeinfo"); # fixme9 this cannot work anymore
	my (@pnode,@nodes);
	@pnode = @nodes = grep { $_ if $RTTInfo{$_}{nodeModel} eq "CiscoRouter" } sort keys %RTTInfo;
	@pnode = @nodes = sort keys %RTTInfo if scalar @nodes == 0;
	unshift @pnode, "";
