...
is_web_available()
{
		# 2024-01-17 OMK-10615 changed from checking against https://opmantek.com/robots.txt to https://firstwave.com/ because it failed vm build 
		# assumed because Pagely.com which hosts opmantek.com and firstwave.com has changed the behaviour for security/performance reasons.
		local IS_WEB_AVAILABLE_URL
		IS_WEB_AVAILABLE_URL=https://firstwave.com/

		printBanner "Checking if Web is accessible..."

		# curl is available even on minimal centos install
		if type curl >/dev/null 2>&1 && execPrint "curl --insecure -L -s -m 10 --retry 2 -o /dev/null $IS_WEB_AVAILABLE_URL 2>/dev/null";
		then
		...
		if type wget >/dev/null 2>&1 && execPrint "wget --no-check-certificate -q -T 10 --tries=3 -O /dev/null $IS_WEB_AVAILABLE_URL 2>/dev/null"; then
				echolog "Web access is OK."
				return 0
		fi
