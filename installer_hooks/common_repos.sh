...
is_web_available()
{
		...
		if type curl >/dev/null 2>&1 && execPrint "curl --insecure -L -s -m 10 --retry 2 -o /dev/null https://firstwave.com/ 2>/dev/null";
		then
		...
		if type wget >/dev/null 2>&1 && execPrint "wget --no-check-certificate -q -T 10 --tries=3 -O /dev/null https://firstwave.com/ 2>/dev/null"; then
				echolog "Web access is OK."
				return 0
		fi
