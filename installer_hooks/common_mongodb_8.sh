...
add_mongo_8_repository () {
		# do mongo?
		if [ "${NO_MONGO}" = 1 ]; then
				echolog "NO_MONGO=${NO_MONGO}: Skipping MongoDB (add_mongo_8_repository) as instructed."
				return 0;
		else
				 echolog "NO_MONGO=${NO_MONGO}: Continuing (add_mongo_8_repository) ...";
		fi;
...
EOF
				...
				# shellcheck disable=SC2039
				local OUTPUT;				
				echolog "Installing gnupg for MongoDB 8";
...
install_mongo_8 () {
		# do mongo?
		if [ "${NO_MONGO}" = 1 ]; then
				echolog "NO_MONGO=${NO_MONGO}: Skipping MongoDB (install_mongo_8) as instructed."
				return 0;
		else
				 echolog "NO_MONGO=${NO_MONGO}: Continuing (install_mongo_8) ...";
		fi;
		...
		printBanner "MongoDB 8 installed."
		return 0
}
...
new_mongo_8_or_bust () {
		# do mongo?
		if [ "${NO_MONGO}" = 1 ]; then
				echolog "NO_MONGO=${NO_MONGO}: Skipping MongoDB (new_mongo_8_or_bust) as instructed."
				return 0;
		else
				 echolog "NO_MONGO=${NO_MONGO}: Continuing (new_mongo_8_or_bust) ...";
		fi;

		# shellcheck disable=SC2039
		local MIN_MAJ MIN_MIN MIN_PATCH

		# Default to 8.0 as we do in add_mongo_8_repository() function
...
EOF
				...
				add_mongo_8_repository "${MIN_MAJ}.${MIN_MIN}"||:;
				install_mongo_8||:;
