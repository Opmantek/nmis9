...
my $adminpwd = NMISNG::Util::decrypt('db_password', $conf->{db_password});
...
if (!$isnoauth)
{
	...
	do
	{
		$adminpwd = input_text("Enter your MongoDB ADMIN password [default: " . NMISNG::Util::decrypt('db_password', $conf->{db_password}) . "]:","18ba");
		$adminpwd = NMISNG::Util::decrypt('db_password', $conf->{db_password}) if ($adminpwd eq "");
