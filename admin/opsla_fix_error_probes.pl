#!/usr/bin/perl
#
## $Id: opsla_fix_error_probes.pl,v 1.2 2012/05/16 05:23:45 keiths Exp $
#
...
foreach my $p ( sort { $a->{probe} cmp $b->{probe} } @probeList ) {
	if ( $p->{rnode} =~ /\d+\.\d+\.\d+\.\d+/ and $p->{raddr} eq "" ) { 
		$IPSLA->updateProbe(
			probe => $p->{probe}, 
			rnode => "other",
			raddr => $p->{rnode}
	...
	if ($p->{status} eq "error") {
		if ( $p->{message} eq "" and $p->{entry} ne "" and $p->{starttime} =~ /probe started at/ ) {
			++$count;
			print $t->elapTime(). " PROBE UPD $count: $p->{probe}, fix status=error\n";
			$IPSLA->updateProbe(
				probe => $p->{probe}, 
				status => "running", 
			);
