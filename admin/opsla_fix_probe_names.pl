#!/usr/bin/perl
#
## $Id: opsla_fix_probe_names.pl,v 1.2 2012/05/16 05:23:45 keiths Exp $
#
...
foreach my $p ( sort { $a->{probe} cmp $b->{probe} } @probeList ) {
	if ($p->{probe} !~ /$p->{pnode}/ or $p->{select} !~ /$p->{pnode}/ ) {
		print $t->elapTime(). " PROBE EXP1: $p->{probe}, pnode=$p->{pnode} select=$p->{select}\n";
		my $probe = $p->{probe};
		if ( $p->{probe} =~ /(.+)_(\d+\.\d+\.\d+\.\d+)_(\w+)_(\d+)/ ) {
			my $pnode = $1;
			my $responder = $2;
			my $operator = $3;
			my $tos = $4;
			my $a = "::";
			print "$pnode$a$responder$a$operator$a$tos\n";
			$IPSLA->updateProbe(
				probe => $p->{probe}, 
				pnode => $pnode,
				select =>  "$pnode$a$responder$a$operator$a$tos"
			);
		}
	}	
	...
	my $select = $p->{select};
	$select =~ s/\:\:/_/g;
	if ($p->{probe} ne $select ) {
		print $t->elapTime(). " PROBE EXP2: $p->{probe}, pnode=$p->{pnode} select=$p->{select}\n";
	}
	if (0) {
		...
		if ( $p->{message} =~ /Error snmpset attributes for probe echo/ ) {
			print $t->elapTime(). " PROBE UPD: $p->{probe}, message=$p->{message}\n";
			$IPSLA->updateProbe(
				probe => $p->{probe}, 
				func => "start",
				timeout => 5
