...
my %ARG = NMISNG::Util::getArguements(@ARGV);
...
my $debug = &NMISNG::Util::setDebug($ARG{debug});
	...
	if ( $dirpass >= 1 and $dirpass < $maxrecurse and $dirlevel <= $maxlevel ) {
		...
		for ( $index = 0; $index <= $#dirlist; ++$index ) {
			...
			if ( -f "$dir/$dirlist[$index]"
				...
				NMISNG::Util::backupFile(file => "$dir/$dirlist[$index]", backup => "$dir/$dirlist[$index].bak" );
