...
my $qrdst = qr/ds\[(BufferHit|BufferMiss)\]\.type/;
...
foreach my $node (sort keys %{$LNT}) {
	...
	if ( getbool($LNT->{$node}{active}) and getbool($LNT->{$node}{collect}) ) {
		...
		print "  ". $t->elapTime(). " Looking for cempMemBuffer databases\n";

		my @instances = $S->getTypeInstances(section => "cempMemBuffer");

		foreach my $index (@instances) 
		{


			my $rrd = $S->getDBName(graphtype => "cempMemBuffer", index => $index);
			
			print "    ". $t->elapTime(). " Found $rrd\n";
	
			# Get the RRD info on the Interface
			my $hash = RRDs::info($rrd);
			++$sum->{count}->{cempMemBuffer};
...
$sum->{count}{'tune-cempMemBuffer'}\tcempMemBuffer RRDs tuned
|;
...
sub initSummary {
	...
	$sum->{count}{node} = 0;
	$sum->{count}{'tune-cempMemBuffer'} = 0;
