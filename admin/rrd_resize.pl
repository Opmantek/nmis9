...
my $C = NMISNG::Util::loadConfTable(conf=>$ARG->{conf},debug=>$ARG->{debug});
my $log_level = $ARG->{debug} // $C->{log_level};
...
sub processDir {
	...
	if ( $dirpass >= 1 and $dirpass < $maxrecurse and $dirlevel <= $maxlevel ) {
		...
		$logger->debug("Found $#dirlist entries");
		...
		for ( $index = 0; $index <= $#dirlist; ++$index ) {
			...
			if ( -f "$dir/$dirlist[$index]"
				...
				$logger->debug("\t$index file $dir/$dirlist[$index]");
...
sub processRRDFile {
	...
	if ( $BACKUP_RRDS ) {
		...
		if ( $backup ) {
			$logger->debug("$rrd backup'ed up to $rrd.bak");
		}
		else {
			print "SKIPPING: \n";
	...
	if ( ( $BACKUP_RRDS and $backup ) or not $BACKUP_RRDS ) {
		...
		NMISNG::Util::setFileProtDiag($rrd); # set file owner/permission, default: nmis, 0775
	}
}
...
sub backupFile {
	...
	if ( not -f $args{backup} ) {			
		if ( -r $args{file} ) {
			open(IN,$args{file}) or warn ("ERROR: problem with file $args{file}; $!");
			open(OUT,">$args{backup}") or warn ("ERROR: problem with file $args{backup}; $!");
			binmode(IN);
			binmode(OUT);
			...
			print STDERR "ERROR: backupFile file $args{file} not readable.\n";
			return 0;
		}
	}
	else {
		print STDERR "ERROR: backup target $args{backup} already exists.\n";
		return 0;
	}
}
