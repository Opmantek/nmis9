#!/usr/bin/perl
#
## $Id: opsla_fix_messages.pl,v 1.2 2012/05/16 05:23:45 keiths Exp $
#
...
foreach my $p ( sort { $a->{probe} cmp $b->{probe} } @probeList ) {
	if ( $p->{status} eq "running like a monkey" and $p->{message} =~ /does not respond/ ){
	...
	elsif ( $p->{message} =~ /error on update rrd database/ ){
		print $t->elapTime(). " PROBE UPD: $p->{probe}, message=$p->{message}\n";
		$IPSLA->updateProbe(
			probe => $p->{probe}, 
			status => "running", 
	...
	elsif ( $p->{status} eq "running" and ($p->{message} eq " " or $p->{message} eq "NULL") ) {
		print $t->elapTime(). " PROBE UPD: $p->{probe}, status=running\n";
		$IPSLA->updateMessage(
			probe => $p->{probe}, 
			message => "NULL", 
		);
