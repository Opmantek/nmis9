...
# Create directories related to NMIS Suite Docker
DOCKER_NMIS_HOME=/home/$DOCKER_USER/nmis-suite
mkdir -p $DOCKER_NMIS_HOME
cd $DOCKER_NMIS_HOME

# Get NMIS Suite configuration files
echo "Pulling docker configuration files from NMIS9 GitHub..."
...
# Update NMIS Suite configuration on script first time run
FLAG_FILE=.init_nmis
if [[ ! -f $FLAG_FILE ]]; then
...
They are used by the NMIS Suite container to connect to the Mongo and Redis instance,
and are stored in the .env file located in $DOCKER_NMIS_HOME. Please backup this file.\n\n"
fi

# Compose up NMIS Suite services and check status
