...
### Addendum: post-audit fix wave (2026-08-28)

The Task 10 gate audits (sink inventory, consumer drive-through, flag-write
audit) passed with residuals that were fixed in five commits before the final
review: setup_mongodb.pl refuses an undecryptable db_password instead of
setting the Mongo password to the literal ciphertext (fc091fdd); the config
editor refuses a password it could not encrypt while encryption is on,
replacing an abort the new contract had made unreachable (08161351); operator
messages no longer reference the enable-eos CLI action removed in 9.4.5
(e22a7e88); the generation test keeps key material out of TAP diagnostics
(ce0f225e); key provisioning is xtrace-safe from generation onward and the
tmp write is noclobber-guarded (2266c4fe). Audit detail lives in the
execution ledger and reports (untracked, .superpowers/sdd/).

Found during the audits, deferred with the other follow-up tickets: the
enable-eos/disable-eos dispatch was commented out of bin/nmis-cli in 9.4.5,
so encryption can only be enabled by hand-editing the config; it must be
restored before OMK-12695 flips the default.
