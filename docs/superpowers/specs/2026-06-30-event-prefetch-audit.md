...
| W3 | `lib/NMISNG.pm:4011,4593,3836` (`Event::save(update=>1)`) | `process_escalations` | escalate-level + notify-list + json-archive updates; no active/historic flip |
...
| W14 | `bin/nmis-cli:1337` (`DB::update historic=1`, act=remove-duplicate-events) | admin CLI `nmis-cli` | archive duplicate active events: `(h0,a1)` -> `(h1,a1)` |
...
| M5 | `lib/NMISNG.pm:4011/4593/3836` escalations `save(update=>1)` | escalate level / notify list / json archive | active | only escalate/notify/json fields change; `(historic,active)` unchanged, so the existence answer is identical -> correct | NO | NO |
...
| M14 | `bin/nmis-cli:1337` CLI remove-duplicate-events | `(h0,a1)` -> `(h1,a1)` of duplicate docs | active | only removes duplicate rows beyond the first; the surviving event keeps its state, so the existence answer for the event is unchanged -> correct. Manual housekeeping | NO | NO |
