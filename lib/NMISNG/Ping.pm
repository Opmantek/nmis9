...
sub ext_ping
{
	...
	{
		NMISNG::Util::logMsg("FATAL: Not yet configured for >$kernel<");
	...
	NMISNG::Util::dbg("ext_ping: $ping{$kernel}",4);
	...
	if ($@)
	{
		...
		if ($C->{debug}>2) {
			$_ = $ping_output;
			s/\n/\n\t\t/g;
			NMISNG::Util::dbg("ERROR: external ping hit timeout $timeout, assuming target $host is unreachable");
			if ($C->{debug}>3) {
				NMISNG::Util::dbg("INFO: The output of the ping command $ping{$^O} was:");
				NMISNG::Util::dbg("\t$_");
			}
		}
	...
	if ($ping_output =~ m@(?:round-trip|rtt)(?:\s+\(ms\))?\s+min/avg/max(?:/(?:m|std)-?dev)?\s+=\s+(\d+(?:\.\d+)?)/(\d+(?:\.\d+)?)/(\d+(?:\.\d+)?)@m) {
		...
		if ($C->{debug}>2) {
			$_ = $ping_output;
			s/\n/\n\t\t/g;
			NMISNG::Util::dbg("ERROR: Could not find ping summary for $host");
			NMISNG::Util::dbg("INFO: The output of the ping command $ping{$^O} was:");
			NMISNG::Util::dbg("\t$_");
			}
		}
	...
	if ($ping_output =~ m@(\d+)% packet loss$@m) {
		...
		if ($C->{debug}>2) {
			$_ = $ping_output;
			s/\n/\n\t\t/g;
			NMISNG::Util::dbg("ERROR: Could not find packet loss summary for $host");;
			NMISNG::Util::dbg("INFO: The output of the ping command $ping{$^O} was:");
			NMISNG::Util::dbg("\t$_");
			}
		}

	NMISNG::Util::dbg("result returning min=$pt{min}, avg=$pt{avg}, max=$pt{max}, loss=$pt{loss}",3);
