...
sub qoskey_standardised_supported_na_string
{
	...
	if (ref($thispolicy) eq "HASH" and $qospolkey)
	{
		...
		elsif ($thispolicy->{CfgSection} eq STANDARDISED_TELDAT_OSDX_QOS_KEY_1 || $thispolicy->{CfgSection} eq STANDARDISED_TELDAT_OSDX_QOS_KEY_2  )
		{
			...
			my @supported_qoskeys = qw(name inout bandwidth action PrePolicyPkt PostPolicyPkt DropByte MaxDropByte PrePolicyByte MaxPrePolicyByte PrePolicyBits MaxPrePolicyBits PostPolicyByte MaxPostPolicyByte PrePolicyPkt_95 PrePolicyByte_95);
			return NA_STR if grep { $qospolkey eq $_ }@supported_qoskeys;
		}
		# teldat qos
