				...
				foreach my $ds ( keys %{$result->{$sect}{$index}} )
				{
					...
					if ( exists( $result->{$sect}->{$index}->{$ds}->{title} ) )
					{
						$self->{view}->{$table}{"${index}_${ds}_value"}
							= rmBadChars( $result->{$sect}->{$index}->{$ds}->{value} );
						$self->{view}{$table}{"${index}_${ds}_title"}
							= rmBadChars( $result->{$sect}->{$index}->{$ds}->{title} );
					}
					...
					{
						dbg( ( $1 eq "SuchObject" ? "ERROR" : "WARNING" ) . ": name=$ds index=$index value=$thisval" );
					...
					dbg( "store: class=$class, type=$sect, DS=$ds, index=$index, value=$thisval", 3 );
				}
		...
		if ( ref( $thissection->{snmp} ) eq "HASH" && $self->{snmp} )
		{
			...
			for my $itemname ( keys %{$thissection->{snmp}} )
			{
				...
				if ( $todos{$itemname} )
				{
					...
					dbg("item $itemname present in multiple sections: " . join( ", ", @{$todos{$itemname}->{section}} ),
						3
					);
				}
				else
...
sub getDBName
{
	...
	if ( defined( $sect = $self->getTypeName( graphtype => $graphtype, index => $index ) ) )
	{
		...
		my $extras = ( defined($index) && $index ne '' ? $NI->{$sect}->{$index} : undef );
