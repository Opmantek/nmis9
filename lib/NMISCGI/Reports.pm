...
our ($q, $Q, $C, $AU, $headeropts, $nmisng, $NT, $GT, $widget, $wantwidget, $nodewrap);
...
sub runcgi
{
	...
	$NT = Compat::NMIS::loadLocalNodeTable();
	...
	$nodewrap = NMISNG::Util::getbool($C->{'wrap_node_names'})? "wrap" : "nowrap";
...
sub top10Report
{
	...
	foreach my $int (NMISNG::Util::sortall(\%interfaceInfo,'node','fwd') )
	{
		if ( NMISNG::Util::getbool($interfaceInfo{$int}{collect}) )
		{
			...
			{
				$S = NMISNG::Sys->new(nmisng => $nmisng);	# can be initialised exactly ONCE
