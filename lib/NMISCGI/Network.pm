...
our ($NT, $GT, @groups, $interface_max_number, $iseditable);
our ($t, $timing, $debug, $nodewrap, $smallGraphHeight, $smallGraphWidth);
our ($network_status, $graphs);
our ($select, $ntwrk, $overallStatus, $overallColor, %icon);
our ($group, $customer, $business);
our ($metricsFirstPeriod, $metricsSecondPeriod, $groupSummary, $start, $end);

sub runcgi {
	my ($args) = @_;
	($q, $Q, $C, $AU, $headeropts) = @{$args}{qw(q Q C AU headeropts)};
	$nmisng = $args->{nmisng};
	$headeropts = {};  #{type=>'text/html',expires=>'now'};

	$interface_max_number = $C->{interface_max_number} || 5000;
	...
	$debug = NMISNG::Util::getbool( $Q->{debug} );
	...
	$network_status = NMISNG::NetworkStatus->new( nmisng => $nmisng );
	$graphs = NMISNG::Graphs->new( nmisng => $nmisng );
	

	# these need loading before the yucky if selection below, which is terminal for some acts
	#my $NT = Compat::NMIS::loadNodeTable();
	
	@groups   = grep { $AU->InGroup($_) } sort $nmisng->get_group_names;
	...
	# graph request
	$ntwrk = ( $select eq 'large' ) ? 'network' : ( $Q->{group} eq '' ) ? 'network' : $Q->{group};
...
escalate ));
	...
	{
		# show the field name when modelling returns nothing.
		my $title = $S->getTitle(attr => $wanted, section => "interface");
		$titles{$wanted} = $title ? $title : $wanted;
	}
