...
our ($q, $Q, $C, $AU, $headeropts, $nmisng, $NT, $GT, $NN);
our ($widget, $wantwidget);
our ($logoutButton, $privlevel, $user);
our ($LL, $logName, $logFileName, $logFileNamePoller, $logNamePoller, $pollerLog);
our ($logSort, $logLines, $logLevel, $logSearch, $logGroup, $logRefresh);
our ($logLevelRequest, $logLevelSummary);
our ($logLinkStart, $logLinkEnd);
...
sub runcgi
{
	...
	# setup global vars
	$LL = undef;		# global ref to conf/Logs.xxxx configuration file
	...
	$NT = Compat::NMIS::loadNodeTable();

	my @groups   = grep { $AU->InGroup($_) } sort $nmisng->get_group_names;
	$GT = { map { $_ => $_ } (@groups) }; # backwards compat; hash assumption sprinkled everywhere
	...
	$logFileName = undef;						# this gets set to the full qualified filename in conf/Logs.xxxx config fiel
	$logFileNamePoller = undef;
	$logNamePoller = undef;

	# defaults
	$logName = $Q->{logname} || 'Event_Log';
	$pollerLog = 'Poller_Event_Log';
	$logSort = defined $Q->{sort} ? $Q->{sort} : 'descending';
	$logLines =  defined $Q->{lines} ? $Q->{lines} : '50' ;
	$logLevel = defined $Q->{level} ? $Q->{level} : 'ALL';
	$logSearch = defined $Q->{search} ? $Q->{search} : '';
	$logGroup = defined $Q->{group} ? $Q->{group} : '';
	$logRefresh = defined $Q->{refresh} ? $Q->{refresh} : '';
