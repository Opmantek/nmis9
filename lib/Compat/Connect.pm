...
#  
# *****************************************************************************
package Compat::Connect;
our $VERSION = "2.0.0";

use strict;
# fixme9: don't think this is useful
# use lib "../../lib";
...
sub curlDataFromRemote {
	...
	my $ST = NMISNG::Util::loadTable(dir=>'conf',name=>'Servers');
	...
	if ( lc($ST->{$server}{name}) eq lc($server) and $ST->{$server}{community} ne "" ) {
		...
		NMISNG::Util::logMsg("ERROR issue with slave $server name or community, check Servers.$ext");
	}
}
...
sub getFileFromRemote {
	...
	if ( $data and $data !~ /SERVER ERROR|504 Gateway Time-out/ ) {
		open(OUT, ">",$file) or NMISNG::Util::logMsg("Could not create $file: $!");
		flock(OUT, LOCK_EX);
		...
		NMISNG::Util::logMsg("ERROR, $args{server} responded with ERROR: $data");
		return 0;		
	}
	else {
		NMISNG::Util::logMsg("ERROR, no node data received from $args{server}");
		return 0;
	}
}
