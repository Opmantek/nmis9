...
my $NT = Compat::NMIS::loadNodeTable(); # load global node table
print "  done in ".$t->deltaTime() ."\n";
...
for my $srv (keys %{$ST}) {
	...
	my $data = Compat::Connect::curlDataFromRemote(server => $srv, func => "sumnodetable", format => "text");
	print "  done in ".$t->deltaTime() ."\n";	
	
}

print $t->markTime(). " loadNodeSummary master=$C->{server_master}\n";
my $NS = Compat::NMIS::loadNodeSummary(master => "true");
print "  done in ".$t->deltaTime() ."\n";	
