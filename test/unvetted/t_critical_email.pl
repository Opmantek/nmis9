...
sub interfaceDown {
	...
	my $NI = $S->{info};
	Compat::NMIS::notify(sys=>$S,event=>"Interface Down",element=>$IF->{$interface_index}{ifDescr},details=>$IF->{$interface_index}{Description}." $critical_text");
...
sub interfaceUp {
	...
	Compat::NMIS::checkEvent(sys=>$S,event=>"Interface Down",level=>"Normal",element=>$IF->{$interface_index}{ifDescr},details=>$IF->{$interface_index}{Description}." $critical_text");
