...
sub update_plugin
{
	...
	my $sub = 'update';
	my $plugin = 'QualityOfServiceStattable.pm';
	...
	if (@$ids)
	{
		...
		for my $id (@$ids)
		{
			...
			if ( defined($inventory_data->{$inventory_data_key}) )
			{
				...
				if (! defined($ifindex))
				{
					$NG->log->info("$plugin:$sub: \$ifindex not defined.");
					return(0,undef);
				}
				...
				$NG->log->info("$plugin:$sub: Found $concept entry with $inventory_data_key '$inventory_data->{$inventory_data_key}': 'ifIndex' = '$inventory_data->{ifIndex}'.");
				$NG->log->info("$plugin:$sub: Found $concept entry with $inventory_data_key '$inventory_data->{$inventory_data_key}': 'Direction' = '$inventory_data->{Direction}'.");
