...
sub update_plugin
{
	...
	$NG->log->info("TeldatBRSStattable.pm:update: Running for node $node");

	# anything to do?
	# this plugin deals only with TeldatBRSStat
	return (0,undef) if (ref($S->{mdl}{systemHealth}{sys}{TeldatBRSStat}) ne "HASH");
	...
	if (@$ids)
	{
		$NG->log->debug9("TeldatBRSStattable.pm:update: \$ids: ".Dumper $ids);
		$NG->log->debug9("TeldatBRSStattable.pm:update: \$S->{mdl}{systemHealth}{sys}{TeldatBRSStat}: ".Dumper \%{$S->{mdl}{systemHealth}{sys}{TeldatBRSStat}});
	        ...
	        {
	                $NG->log->error("TeldatBRSStattable.pm:update: Failed to get inventory: $error");
	                return(0,undef);
	        }
	        my %ifdata =  map { ($_->{data}->{index} => $_->{data}) } (@{$result->data});
		$NG->log->debug9("TeldatBRSStattable.pm:update: \%ifdata: ".Dumper \%ifdata);
		...
		for my $id (@$ids)
		{
			...
			if ( defined($inventory_data->{ifIndex}) )
			{
	                        ...
	                        if ( defined $ifdata{$ifindex}{ifDescr} )
				{
					...
					$NG->log->debug("TeldatBRSStattable.pm:update: Node $node updating node info TeldatBRSStat $inventory_data->{index}: new '$inventory_data->{ifDescr}'");
				}
				else
				{
	...
	{
		$NG->log->debug("TeldatBRSStattable.pm:update: 'if(\@\$ids)' returned false");
	}
