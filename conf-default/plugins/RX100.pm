...
sub update_plugin
{
    ...
    $NG->log->info("Running update_plugin RX100 for node $node");
	...
	return (1,undef) if ( $catchall_data->{nodeModel} ne "ZTE-RX100" or !NMISNG::Util::getbool($catchall_data->{collect}));
	...
	{ 
		...
		my $ONTDescr_data = $snmp->getindex("1.3.6.1.4.1.3902.1082.500.20.2.1.2.1.19",$max_repetitions);
		...
		#		  },
        my $ONT_SerialNumber = $snmp->getindex("1.3.6.1.4.1.3902.1082.500.20.2.1.2.1.3",$max_repetitions);
        ...
        if (@$GponUserTrafficIds)
        {
            for my $GponUserTrafficId (@$GponUserTrafficIds)
            {
                ...
                if($data->{MultiSerUserPara} == 100 || $data->{MultiSerUserPara} == 400) {
                    ...
                    my $card = $data->{zteExtSrvFlowPara2}; 
                    my $port = $data->{zteExtSrvFlowPara3};
                    my $ONT_ID = $data->{zteExtSrvFlowPara4};
		...
		{	
			...
			for my $sectionId (@$sectionIds)
			{
				...
				if ( $data->{zteGponDeviceOntPassword} ) {
					my $d = $data->{zteGponDeviceOntPassword};
					$d =~ s/[^\d]//g;
					# Save if we have made changes
					if ($d ne $data->{zteGponDeviceOntPassword})
					{
						$data->{zteGponDeviceOntPassword} = $d;
